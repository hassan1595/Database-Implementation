...
//        change adaptation
    ...
    {
        System.arraycopy(bufsrc , 0, bufdest
                , 0, bufdest.length);
    }
    ...
    private boolean checkBlist(ArrayList<Pair<Integer, Integer>> Blist, int resourceId, int pageNum)
    {
        ...
        {
            if(resourceIdPageNum.getFirst() == resourceId
            && resourceIdPageNum.getSecond() == pageNum)
            {
                return true;
            }
        }
    ...
    private boolean checkTList(ArrayList<CacheData> Tlist, int resourceId, int pageNum)
    {
        for(CacheData cd:Tlist)
        {
            if(cd.getResourceId() == resourceId &&
                    cd.getPage().getPageNumber() == pageNum)
            {
                return true;
            }
        }
    ...
    private void evictBlists(int ResourceId, int pageNum){
        ...
        if(checkBlist(this.listB1, ResourceId, pageNum)
        || checkBlist(this.listB2, ResourceId, pageNum) )
            return;

        if(this.listT1.size() + this.listB1.size() == this.numPages
        && this.listT1.size()  < this.numPages)
        {
        ...
        if(this.listT1.size() + this.listB1.size() <this.numPages
        && this.listT1.size() + this.listB1.size() +this.listT2.size() + this.listB2.size() == 2*this.numPages )
        {
    ...
    private void addToTAndChangeAdapt(CacheData cd)
    {
        if(checkBlist(this.listB1, cd.getResourceId(), cd.getPage().getPageNumber()) ||
                checkBlist(this.listB2,  cd.getResourceId(), cd.getPage().getPageNumber())) {
        ...
        if(checkBlist(this.listB1, cd.getResourceId(), cd.getPage().getPageNumber()))
        {
            this.adaptation = (float) Math.min(this.numPages, this.adaptation +
                    (this.listB1.size() > this.listB2.size() ? 1.0 : ((float) this.listB2.size() / this.listB1.size())));

        }
        if(checkBlist(this.listB2, cd.getResourceId(), cd.getPage().getPageNumber()))
        {
            this.adaptation = (float) Math.max(0.0, this.adaptation -
                    (this.listB2.size() > this.listB1.size() ? 1.0 : ((float) this.listB1.size() / this.listB2.size())));
...
//    replaces the best cache entry and returns it
    private EvictedCacheEntry getAndAdapt(CacheableData newPage, int resourceId ,int freeEvictIndex,
    boolean T1prio, boolean pin) throws CachePinnedException {
...
//            DO NOT WRITE PAGE FIRST
    ...
    public EvictedCacheEntry addPagepinOpt(CacheableData newPage, int resourceId, boolean pin) throws DuplicateCacheEntryException, CachePinnedException {
        ...
        if(checkTList(this.listT1, resourceId, newPage.getPageNumber())
        || checkTList(this.listT2, resourceId, newPage.getPageNumber() ))
        {
...
//        case a
        if( !this.listT1.isEmpty() &&
                (this.listT1.size() > this.adaptation ||
                (this.listT1.size() == this.adaptation && checkBlist(this.listB2, resourceId, newPage.getPageNumber()))
                ))
        {
